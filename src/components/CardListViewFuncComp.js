...
export const CardListViewFunc = ({onChangeColor}) => {
    const ref = useRef();
...
*/
    ...
    const onOpenCard = (isCustom) => {
        if (isCustom) {
            navigation.push('AddCategory', {currentCategory});
        } else {
            navigation.push('TaskList', {currentCategory});
        }
    };
    ...
    function Item({categoryKey}) {
        ...
        return (
            <View>
                <Text style={{color: 'white', fontSize: 16, paddingBottom: 6}}>{taskTitle}</Text>
            <TouchableOpacity activeOpacity={0.8}
                              onPress={onOpenCard.bind(this, isCustom)}>
                <Card style={{
                    borderRadius: 8,
                    elevation: 2,
    ...
    return (
        <>
            <FlatList
                ...
                durationRate={'fast'}
                snapToInterval={screenW - 90}
                style={{flex: 1, flexDirection: 'row'}}
                horizontal={true}
                data={data}
                renderItem={({item}) => <Item categoryKey={item}/>}
